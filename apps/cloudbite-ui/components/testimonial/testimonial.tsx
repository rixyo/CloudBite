import React from 'react';
import Image from 'next/image';

const Testimonial:React.FC = () => {
    
    return (
      <div className="flex items-center md:gap-14 p-5">
        <div className="hidden lg:block lg:w-[30rem] lg:h-[40rem]  bg-no-repeat rounded-xl   bg-[url('/img/chef.png')] bg-[#53EC62]" />
<div>
  <Image src={'/img/chef.png'} width={500} height={500} alt='' className='lg:hidden'/>
</div>
        <div className="flex items-center">
          <div className="flex-col">
            <div>
              <h1 className="font-[700] leading-normal tracking-[0.21875rem] uppercase text-[#FF6868] text-[1.25rem]">
                Testimonials
              </h1>
            </div>
            <div>
              <h1 className="font-[700]   lg:leading-[4.91rem]  lg:tracking-[0.21875rem] max-w-[50%] md:max-w-[80%] lg:max-w-[70%] uppercase text-[#000] text-[1.5rem] md:text-[3.75rem]">
                What Our Customers Say About Us
              </h1>
            </div>
            <div className="w-[40rem] p-5">
              <p className="font-[500] md:leading-[2.5rem] md:tracking-[0.21875rem] max-w-[55%] md:max-w-[85%]  text-[#555] text-[1.625rem]">
                “I had the pleasure of dining at Foodi last night, and I &apos;m
                still raving about the experience! The attention to detail in
                presentation and service was impeccable“
              </p>
            </div>
            <div className="md:flex items-center">
              <div className="flex items-center  rounded-full p-2">
                <div className="">
                  <Image
                    src="/img/rv3.png"
                    width={50}
                    height={50}
                    alt=""
                    className="border-2  rounded-full"
                  />
                </div>
                <Image
                  src="/img/rv1.png"
                  width={50}
                  height={50}
                  alt=""
                  className="border-2 rounded-full"
                />
                <div className="">
                  <Image
                    src="/img/rv2.png"
                    width={50}
                    height={50}
                    alt=""
                    className="border-2 rounded-full"
                  />
                </div>
              </div>
              <div className="flex items-center">
                <div className="flex-col">
                  <div className="">
                    <h1 className="font-[600]  uppercase text-[#000] text-[1.75rem]">
                      Customer Feedback
                    </h1>
                  </div>
                  <div className="flex items-center">
                    <div className="w-[1.5rem] h-[1.5rem]">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="24"
                        height="24"
                        viewBox="0 0 24 24"
                        fill="none"
                      >
                        <g clip-path="url(#clip0_1_69)">
                          <path
                            d="M1.3268 12.4003L4.8868 15.0003L3.5348 19.1873C3.31631 19.8367 3.31354 20.5393 3.52691 21.1904C3.74027 21.8415 4.15834 22.4062 4.7188 22.8003C5.26965 23.2071 5.93719 23.425 6.62195 23.4216C7.30671 23.4182 7.97204 23.1936 8.5188 22.7813L11.9998 20.2193L15.4818 22.7783C16.0317 23.1828 16.6956 23.4025 17.3782 23.4058C18.0607 23.4091 18.7268 23.1959 19.2806 22.7968C19.8343 22.3978 20.2473 21.8334 20.4601 21.1848C20.6729 20.5362 20.6745 19.8369 20.4648 19.1873L19.1128 15.0003L22.6728 12.4003C23.2219 11.9988 23.6302 11.4341 23.8391 10.7867C24.0481 10.1394 24.0472 9.44251 23.8365 8.79571C23.6258 8.14891 23.216 7.58525 22.6658 7.18523C22.1156 6.7852 21.4531 6.56929 20.7728 6.56832H16.3998L15.0728 2.43232C14.8641 1.78128 14.4541 1.21333 13.9018 0.810386C13.3495 0.407437 12.6835 0.190308 11.9998 0.190308C11.3161 0.190308 10.6501 0.407437 10.0978 0.810386C9.54553 1.21333 9.13548 1.78128 8.9268 2.43232L7.5998 6.56832H3.2308C2.55054 6.56929 1.88799 6.7852 1.33778 7.18523C0.787564 7.58525 0.377837 8.14891 0.167118 8.79571C-0.0436018 9.44251 -0.0445344 10.1394 0.164453 10.7867C0.37344 11.4341 0.781657 11.9988 1.3308 12.4003H1.3268Z"
                            fill="#FFE605"
                          />
                        </g>
                        <defs>
                          <clipPath id="clip0_1_69">
                            <rect width="24" height="24" fill="white" />
                          </clipPath>
                        </defs>
                      </svg>
                    </div>
                    <div>
                      <h1 className="font-[500]  uppercase text-[#807E7E] text-[1.5rem]">
                        (18.6k Reviews)
                      </h1>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    );
}
export default Testimonial;